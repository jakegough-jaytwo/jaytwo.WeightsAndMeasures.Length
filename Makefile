...
DOCKER_TAG?=jaytwo_weightsandmeasures_length
...
build: restore
	dotnet build ./jaytwo.WeightsAndMeasures.Length.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.WeightsAndMeasures.Length.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.WeightsAndMeasures.Length.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.WeightsAndMeasures.Length; \
